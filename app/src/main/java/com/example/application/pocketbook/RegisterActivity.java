    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        registerButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                else if(!(password.isEmpty() &&  email.isEmpty() && confirmPass.isEmpty())){
                    mFirebaseAuth.createUserWithEmailAndPassword(email,password).addOnCompleteListener(RegisterActivity.this, new OnCompleteListener<AuthResult>() {
                        @Override
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            if (email.toString().trim().matches(emailPattern) && PASSWORD_PATTERN.matcher(password).matches() && password.equals(confirmPass) ) {
                                ...
                                emailText2.setError("Invalid Email");
                                emailText2.requestFocus();
                            }
                            else if(!PASSWORD_PATTERN.matcher(password).matches()){
                                passwordText2.setError("Invalid Password");
                                passwordText2.requestFocus();
                            }
